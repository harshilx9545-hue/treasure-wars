 ...
 * attack / block / bow-draw / walk-bob animations. Purely cosmetic.
 */
export class ViewModel {
  ...
  update(dt: number, walking: boolean, sprinting: boolean, onGround: boolean): void {
    ...
    // Bow draw (pull toward the eye as it charges).
    if (isRanged && this.bowCharge > 0) {
      pz += this.bowCharge * 0.1; px -= this.bowCharge * 0.05;
      rz += this.bowCharge * 0.1;
    }
